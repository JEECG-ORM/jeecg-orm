...
<script>
    import {addCourseApply, editCourseApply,duplicateCheck,queryColumnList} from '@/api/api'
    import Area from '@/components/_util/Area'
    export default {
        name: "CourseApplyModal",
        data() {
            return {
                modalWidth: 800,
                drawerWidth: 700,
            ...
            handleSubmit() {
                ...
                this.$refs.form.validate(valid => {
                    if (valid) {
                        ...
                        let obj;
                        if (!this.model.id) {
                            obj = addCourseApply(this.model);
                        } else {
                            obj = editCourseApply(this.model);
                        }
                        obj.then((res) => {
                            if (res.success) {
