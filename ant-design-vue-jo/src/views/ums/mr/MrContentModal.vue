        ...
        <a-spin :spinning="confirmLoading">
            <a-form-model ref="form" :model="model" :rules="validatorRules">
                <a-form-model-item v-if="item.isInsert" v-for="item in columns" :label="item.columnComment"
                    ...
                    <j-dict-select-tag  v-if="item.htmlType==='select'" v-model="model[item.javaField]"  :dictCode="item.dictCode" :placeholder="'请选择'+item.columnComment"/>
                </a-form-model-item>
            </a-form-model>
        </a-spin>
...
<script>
    import {addMrContent, editMrContent,duplicateCheck,queryColumnList} from '@/api/api'
            ...
            handleSubmit() {
                ...
                this.$refs.form.validate(valid => {
                    if (valid) {
                        ...
                        let obj;
                        if (!this.model.id) {
                            obj = addMrContent(this.model);
                        } else {
                            obj = editMrContent(this.model);
                        }
                        obj.then((res) => {
                            if (res.success) {
