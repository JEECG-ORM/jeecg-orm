...
export const JeecgListMixin = {
  ...
  methods:{
    ...
    handleStatus: function (checked,id) {
      if(!this.url.status){
        this.$message.error("请设置url.status属性!")
        return
      }
      var that = this;
      postAction(that.url.status, {checked:checked,id: id}).then((res) => {
        if (res.success) {
          //重新计算分页问题
          that.reCalculatePage(1)
          //that.$message.success(res.message);
    ...
    handleField: function (field,checked,id) {
      if(!this.url.field){
        this.$message.error("请设置url.status属性!")
        return
      }
      var that = this;
      postAction(that.url.field+"/"+field, {checked:checked,id: id}).then((res) => {
        if (res.success) {
          //重新计算分页问题
          that.reCalculatePage(1)
          //that.$message.success(res.message);
