...
public class DictAspect {
    ...
    // 定义切点Pointcut
    @Pointcut("execution(public * com.hongru.*.controller.*Controller.*(..))")
...
/**
     * 本方法针对返回对象为Result 的HongRuPage的分页列表数据进行动态字典注入
    ...
    private void parseDictText(Object result) {
        if (result instanceof Result) {
            if (((Result) result).getResult() instanceof HongRuPage) {
                ...
                for (Object record : ((HongRuPage) ((Result) result).getResult()).getRecords()) {
                ...
                ((HongRuPage) ((Result) result).getResult()).setRecords(items);
            }
    ...
    private Map<String, List<DictModel>> translateAllDict(Map<String, List<String>> dataListMap) {
        ...
        for (String dictCode : dataListMap.keySet()) {
            ...
            for (String s : dataList) {
                ...
                if (dictCode.contains(",")) {
                    ...
                    if (redisTemplate.hasKey(keyString)) {
                        try {
                            String text =   StringUtil.getString( redisTemplate.opsForValue().get(keyString));
            ...
            if (needTranslDataTable.size() > 0) {
                ...
                String sql="select " + code + " as value ," + text + " as text from " + table + " where " + code + " in('" + values + "')";
                log.info("translateDictFromTableByKeys.values:" + values);
                List<DictModel> texts = DB.findDto(DictModel.class, sql).findList();
                List<DictModel> list = translText.computeIfAbsent(dictCode, k -> new ArrayList<>());
                list.addAll(texts);
                // 做 redis 缓存
