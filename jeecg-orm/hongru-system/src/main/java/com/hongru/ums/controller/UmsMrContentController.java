...
import com.hongru.ums.entity.UmsMrContent;
...
*/
@RestController
@RequestMapping("/ums/mr/content")
@Slf4j
@ApiIgnore
public class UmsMrContentController {

    @PostMapping("/list")
    @ApiOperation("报告内容列表")
    @ApiOperationSupport(params = @DynamicParameters(properties = {
                @DynamicParameter(name = "title", value = "标题",example = "综合检测分析"),
    }))
    ...
    @ApiOperation("添加")
    @ApiOperationSupport(ignoreParameters = {"umsMrContent.id"})
    @PostMapping(value = "/add")
    public Result<UmsMrContent> add(@RequestBody UmsMrContent umsMrContent) {
        umsMrContent.save();
        return Result.OK("添加成功");
    }

    @PostMapping(value = "/edit")
    public Result<UmsMrContent> edit(@RequestBody UmsMrContent umsMrContent) {
        umsMrContent.update();
